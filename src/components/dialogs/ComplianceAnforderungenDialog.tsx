...
interface ComplianceAnforderungenDialogProps {
  open: boolean;
  onClose: () => void;
  onSubmit: (fields: ComplianceAnforderungen['fields']) => Promise<void>;
  defaultValues?: ComplianceAnforderungen['fields'];
  sicherheitskontrollenList: Sicherheitskontrollen[];
  enablePhotoScan?: boolean;
  enablePhotoLocation?: boolean;
}

export function ComplianceAnforderungenDialog({ open, onClose, onSubmit, defaultValues, sicherheitskontrollenList, enablePhotoScan = true, enablePhotoLocation = true }: ComplianceAnforderungenDialogProps) {
  const [fields, setFields] = useState<Partial<ComplianceAnforderungen['fields']>>({});
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      const clean = cleanFieldsForApi({ ...fields }, 'compliance_anforderungen');
      await onSubmit(clean as ComplianceAnforderungen['fields']);
      onClose();
    } finally {
      setSaving(false);
    }
  ...
  async function handlePhotoScan(file: File) {
    ...
    try {
      ...
      contextParts.push(`<available-records field="verknuepfte_kontrollen" entity="Sicherheitskontrollen">\n${JSON.stringify(sicherheitskontrollenList.map(r => ({ record_id: r.record_id, ...r.fields })), null, 2)}\n</available-records>`);
      if (usePersonalInfo) {
        try {
      ...
      const schema = `{\n  "anforderung_name": string | null, // Name der Anforderung\n  "anforderung_id": string | null, // Anforderungs-ID\n  "rahmenwerk": LookupValue | null, // Rahmenwerk / Standard (select one key: "iso_27001" | "dsgvo" | "bsi_it_grundschutz" | "soc_2" | "pci_dss" | "nist" | "intern" | "sonstiges") mapping: iso_27001=ISO 27001, dsgvo=DSGVO, bsi_it_grundschutz=BSI IT-Grundschutz, soc_2=SOC 2, pci_dss=PCI DSS, nist=NIST, intern=Intern, sonstiges=Sonstiges\n  "anforderung_beschreibung": string | null, // Beschreibung\n  "prioritaet": LookupValue | null, // Priorität (select one key: "niedrig" | "mittel" | "hoch" | "kritisch") mapping: niedrig=Niedrig, mittel=Mittel, hoch=Hoch, kritisch=Kritisch\n  "faelligkeitsdatum": string | null, // YYYY-MM-DD\n  "compliance_status": LookupValue | null, // Compliance-Status (select one key: "konform" | "teilweise_konform" | "nicht_konform" | "in_bearbeitung" | "nicht_anwendbar") mapping: konform=Konform, teilweise_konform=Teilweise konform, nicht_konform=Nicht konform, in_bearbeitung=In Bearbeitung, nicht_anwendbar=Nicht anwendbar\n  "compliance_verantwortlicher_vorname": string | null, // Vorname des Verantwortlichen\n  "compliance_verantwortlicher_nachname": string | null, // Nachname des Verantwortlichen\n  "verknuepfte_kontrollen": string | null, // Display name from Sicherheitskontrollen (see <available-records>)\n}`;
      ...
      setFields(prev => {
        ...
        const applookupKeys = new Set<string>(["verknuepfte_kontrollen"]);
        for (const [k, v] of Object.entries(raw)) {
          if (applookupKeys.has(k)) continue;
          if (v != null) merged[k] = v;
        }
        const verknuepfte_kontrollenName = raw['verknuepfte_kontrollen'] as string | null;
        if (verknuepfte_kontrollenName) {
          const verknuepfte_kontrollenMatch = sicherheitskontrollenList.find(r => matchName(verknuepfte_kontrollenName!, [String(r.fields.kontrolle_name ?? '')]));
          if (verknuepfte_kontrollenMatch) merged['verknuepfte_kontrollen'] = createRecordUrl(APP_IDS.SICHERHEITSKONTROLLEN, verknuepfte_kontrollenMatch.record_id);
        }
        return merged as Partial<ComplianceAnforderungen['fields']>;
      });
      setScanSuccess(true);
  ...
  const DIALOG_INTENT = defaultValues ? 'Compliance-Anforderungen bearbeiten' : 'Compliance-Anforderungen hinzufügen';
    ...
    <Dialog open={open} onOpenChange={v => !v && onClose()}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="verknuepfte_kontrollen">Verknüpfte Sicherheitskontrollen</Label>
            <Select
              value={extractRecordId(fields.verknuepfte_kontrollen) ?? 'none'}
              onValueChange={v => setFields(f => ({ ...f, verknuepfte_kontrollen: v === 'none' ? undefined : createRecordUrl(APP_IDS.SICHERHEITSKONTROLLEN, v) }))}
            >
              <SelectTrigger id="verknuepfte_kontrollen"><SelectValue placeholder="Auswählen..." /></SelectTrigger>
              <SelectContent>
                <SelectItem value="none">—</SelectItem>
